...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
     ...
     //  var bg:FlxSprite = new FlxSprite().loadGraphic(Paths.image('newMenu/bg'));
		bg.antialiasing = ClientPrefs.globalAntialiasing;
		// bg.setGraphicSize(Std.int(bg.width * 0.6));
		bg.updateHitbox();
		add(bg);
		bg = new FlxSprite(titleJSON.bgx, titleJSON.bgy);

		logoBl = new FlxSprite(titleJSON.titlex, titleJSON.titley);
		logoBl.frames = Paths.getSparrowAtlas('newMenu/logoV2');
		...
		switch(easterEgg.toUpperCase())
		{
			...
			//EDIT THIS ONE IF YOU'RE MAKING A SOURCE CODE MOD!!!!
			//EDIT THIS ONE IF YOU'RE MAKING A SOURCE CODE MOD!!!!
				gfDance.frames = Paths.getSparrowAtlas('gf_Title');
		...
		if (!skippedIntro)
		{
			if (playJingle) //Ignore deez
			{
				...
				switch(easteregg)
				{
					...
					case 'WHITMORE':
					PlayState.SONG = Song.loadFromJson("air-rock", "week1");
